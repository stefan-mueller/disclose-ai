...
step2Label = el("div", {}, [
  ...
  el("p", { class: "gaidet-tool-instructions" }, "First specify in Step 1 what you're allowed to do with AI. Then select and describe your use of AI for one or more tasks below.")
])
```
...
form = {
  ...
  return locked ? lockBox(categoryEl, `Locked – tick “${cat.label} in Step 1 above to unlock this category.`) : categoryEl;
  })));
}
```
...
**Analytics.** This site uses [Cabin](https://withcabin.com), a privacy-focused analytics service, to understand overall visitor numbers. Cabin does not use cookies, does not track IP addresses, and cannot identify individual visitors — it only reports aggregate, anonymous traffic data (e.g. how many people visited, which pages). In addition to page views, we log one aggregate, anonymous count each time someone clicks "Yes, generate declaration" (not on subsequent edits), so we can tell how many visitors complete a declaration; this is technically recorded as a page view to a dummy page (`/count/generate-declaration.html`) because Cabin's plan does not support custom events. See [Cabin's privacy policy](https://withcabin.com/privacy/disclose-ai.app) for details. No data about the content you enter into this tool is ever sent to Cabin or anyone else.
